...
export class SubmoduleEc2Instance {
  constructor(private ec2Client: EC2Client, private config: Ec2InstanceConfig) {}
  ...
  public async create(): Promise<void> {
    ...
    const runCmd = new RunInstancesCommand({
      ...
      UserData: this.config.userData ? Buffer.from(this.config.userData).toString("base64") : undefined,
      InstanceMarketOptions:
        this.config.mode === "spot" ? { MarketType: "spot" } : undefined,
    });
  ...
  public async start(): Promise<void> {
    ...
    await this.ec2Client.send(
      new StartInstancesCommand({ InstanceIds: [this.config.id] })
    );
  }
  ...
  public async stop(): Promise<void> {
    if (!this.config.id) throw new Error("No instance ID.");
    await this.ec2Client.send(
      new StopInstancesCommand({ InstanceIds: [this.config.id] })
    );
  }
  ...
  public async terminate(): Promise<void> {
    if (!this.config.id) throw new Error("No instance ID.");
    await this.ec2Client.send(
      new TerminateInstancesCommand({ InstanceIds: [this.config.id] })
    );
  }
}
